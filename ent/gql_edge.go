...
func (e *Exercise) Routines(ctx context.Context) (result []*Routine, err error) {
	if fc := graphql.GetFieldContext(ctx); fc != nil && fc.Field.Alias != "" {
		result, err = e.NamedRoutines(graphql.GetFieldContext(ctx).Field.Alias)
	} else {
		result, err = e.Edges.RoutinesOrErr()
	}
	if IsNotLoaded(err) {
		result, err = e.QueryRoutines().All(ctx)
	}
	return result, err
}
...
func (r *Routine) Exercises(ctx context.Context) (result []*Exercise, err error) {
	if fc := graphql.GetFieldContext(ctx); fc != nil && fc.Field.Alias != "" {
		result, err = r.NamedExercises(graphql.GetFieldContext(ctx).Field.Alias)
	} else {
		result, err = r.Edges.ExercisesOrErr()
	}
	if IsNotLoaded(err) {
		result, err = r.QueryExercises().All(ctx)
	}
	return result, err
}
