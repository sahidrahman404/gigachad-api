...
func (eq *EquipmentQuery) QueryExercises() *ExerciseQuery {
...
func (eq *EquipmentQuery) Clone() *EquipmentQuery {
	...
	return &EquipmentQuery{
		...
		withExercises: eq.withExercises.Clone(),
...
func (eq *EquipmentQuery) WithExercises(opts ...func(*ExerciseQuery)) *EquipmentQuery {
	query := (&ExerciseClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	eq.withExercises = query
	return eq
}
...
func (eq *EquipmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Equipment, error) {
	...
	if query := eq.withExercises; query != nil {
...
func (eq *EquipmentQuery) loadExercises(ctx context.Context, query *ExerciseQuery, nodes []*Equipment, init func(*Equipment), assign func(*Equipment, *Exercise)) error {
	...
	byID := make(map[pksuid.ID]*Equipment)
	nids := make(map[pksuid.ID]map[*Equipment]struct{})
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Where(sql.InValues(joinT.C(equipment.ExercisesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(equipment.ExercisesPrimaryKey[0]))
