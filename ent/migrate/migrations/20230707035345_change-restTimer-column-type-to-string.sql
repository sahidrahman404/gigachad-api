...
CREATE TABLE `new_routine_exercises` (`id` text NOT NULL, `rest_timer` text NULL, `sets` json NOT NULL, `routine_id` text NOT NULL, `exercise_id` text NOT NULL, `user_id` text NULL, PRIMARY KEY (`id`), CONSTRAINT `routine_exercises_routines_routines` FOREIGN KEY (`routine_id`) REFERENCES `routines` (`id`) ON DELETE CASCADE, CONSTRAINT `routine_exercises_exercises_exercises` FOREIGN KEY (`exercise_id`) REFERENCES `exercises` (`id`) ON DELETE CASCADE, CONSTRAINT `routine_exercises_users_routine_exercises` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE);
