...
CREATE TABLE `new_routine_exercises` (`id` text NOT NULL, `rest_time` text NULL, `sets` json NOT NULL, `routine_id` text NOT NULL, `exercise_id` text NOT NULL, `user_id` text NOT NULL, PRIMARY KEY (`id`), CONSTRAINT `routine_exercises_routines_routines` FOREIGN KEY (`routine_id`) REFERENCES `routines` (`id`) ON DELETE CASCADE, CONSTRAINT `routine_exercises_exercises_exercises` FOREIGN KEY (`exercise_id`) REFERENCES `exercises` (`id`) ON DELETE CASCADE, CONSTRAINT `routine_exercises_users_routine_exercises` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE);
-- Copy rows from old table "routine_exercises" to new temporary table "new_routine_exercises"
INSERT INTO `new_routine_exercises` (`id`, `sets`, `routine_id`, `exercise_id`, `user_id`) SELECT `id`, `sets`, `routine_id`, `exercise_id`, `user_id` FROM `routine_exercises`;
